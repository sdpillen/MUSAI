...
%
% MUPEX
% [EEG] = MUPEX_EEGamplitudecalibration(tg,info)
% by Til Ole Bergmann (mail@tobergmann.de)
% last edited 2017/10/04 by TOB
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [results, data, EEG] = MUPEX_EEGcalibration(tg,info)
...
for i = 1:info.numEpochs % loop over trials   
    ...
    if info.development_flag == 0
        data.trial{i} = mc_triggerandrecordEEG(tg,times,stimulators,marker); % trigger stimultor(s) and receive data
...
ylabel('Power (mV²)');
set(gca,'XTick',[cfg.foi]);
title({'1/f corrected (power*f^2) PowerTargetSignal', ['Individual µ peak frequency: ' num2str(EEG.mufreq) ' Hz']});
...
% EEG.lowAmpLim = EEG.binranges(find(cumsum(mean(EEG.bincounts,1)) > sum(mean(EEG.bincounts,1))*info.lowAmpLimCrit,1,'first')); % low amp lower threshold
% EEG.lowAmpThresh = EEG.binranges(find(cumsum(mean(EEG.bincounts,1)) > sum(mean(EEG.bincounts,1))*info.lowAmpCrit,1,'first')); % low amp upper  threshold
% EEG.highAmpThresh = EEG.binranges(find(cumsum(mean(EEG.bincounts,1)) < sum(mean(EEG.bincounts,1))*info.highAmpCrit,1,'last')); % high amp lower threshold
% EEG.highAmpLim = EEG.binranges(find(cumsum(mean(EEG.bincounts,1)) < sum(mean(EEG.bincounts,1))*info.highAmpLimCrit,1,'last')); % high amp upper threshold
